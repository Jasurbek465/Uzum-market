...
const getPro = async () => {
    const url = "url qo'yish kerak jsonnikini";
    const uniOrabTurgan = document.querySelector("id bolishi shart!!!");
    ...
    try {
        ...
        dat.forEach(item => {
            uniOrabTurgan.innerHTML += `
            qilinadigan malumot kiritiladi
            `;
        });
    } catch (error) {
        console.log(error);
